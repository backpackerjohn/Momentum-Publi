...
export const getAnchorColor = (title: string, returnColorOnly: boolean = false): string => {
    const lowerTitle = title.toLowerCase();
    
    let colorVar: string;
    let classSet: string;

    if (lowerTitle.includes('work') || lowerTitle.includes('school') || lowerTitle.includes('meeting') || lowerTitle.includes('deep work') || lowerTitle.includes('focus') || lowerTitle.includes('study')) {
        colorVar = 'var(--color-secondary-accent)';
        classSet = 'bg-green-100 text-green-900 border-l-4 border-green-400';
    } else if (lowerTitle.includes('gym') || lowerTitle.includes('workout') || lowerTitle.includes('health') || lowerTitle.includes('fitness') || lowerTitle.includes('doctor')) {
        colorVar = 'var(--color-warning)';
        classSet = 'bg-yellow-100 text-yellow-900 border-l-4 border-yellow-400';
    } else if (lowerTitle.includes('family') || lowerTitle.includes('kids') || lowerTitle.includes('social') || lowerTitle.includes('date')) {
        colorVar = 'var(--color-primary-accent)';
        classSet = 'bg-red-100 text-red-900 border-l-4 border-red-400';
    } else {
        colorVar = 'var(--color-border-hover)';
        classSet = 'bg-[var(--color-surface-sunken)] border-l-4 border-[var(--color-border-hover)] text-[var(--color-text-secondary)]';
    }
    
    return returnColorOnly ? colorVar : classSet;
};
