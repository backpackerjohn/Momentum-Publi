...
const TaskPage: React.FC<TaskPageProps> = ({ savedTasks, setSavedTasks, onResume, onUndo }) => {
  ...
  const renderTaskCard = (task: SavedTask) => {
    ...
    return (
      <div key={task.id} className="bg-[var(--color-surface)] p-6 rounded-[var(--border-radius-xl)] shadow-sm border border-[var(--color-border)] flex flex-col transition-all duration-300 hover:shadow-md hover:border-[var(--color-border-hover)]">
        {isEditing ? (
            <div className="flex-1">
                ...
                <textarea
                    ...
                    className="w-full mt-3 p-2 border border-[var(--color-border-hover)] rounded-[var(--border-radius-lg)] text-sm focus:ring-1 focus:ring-[var(--color-primary-accent)] focus:border-[var(--color-primary-accent)] bg-transparent"
                    autoFocus
                    rows={3}
                />
            </div>
            ...
            <div className="flex-1 cursor-pointer" onClick={() => handleResume(task)}>
                ...
                {task.note && task.note !== 'No note added.' ? (
                    <p className="mt-2 text-sm text-[var(--color-text-secondary)] italic">"{task.note}"</p>
                ) : (
                    <p className="mt-2 text-sm text-[var(--color-text-subtle)] italic">No note added.</p>
                )}
        ...
        <div className="mt-4 pt-4 border-t border-[var(--color-border)]/80 flex items-center gap-2">
            {isEditing ? (
                <>
                    <button onClick={() => setEditingTask(null)} className="px-4 py-2 text-sm font-bold text-[var(--color-text-secondary)] bg-[var(--color-surface-sunken)] hover:bg-[var(--color-border)] rounded-full transition-all">Cancel</button>
                    <button onClick={handleSaveEdit} className="flex-1 px-4 py-2 text-sm font-semibold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] hover:bg-[var(--color-primary-accent-hover)] rounded-full transition-all">Save Changes</button>
                </>
            ) : (
                <>
                    <button onClick={(e) => { e.stopPropagation(); setEditingTask({ id: task.id, note: task.note, nickname: task.nickname || '' }); }} className="px-4 py-2 text-sm font-bold text-[var(--color-text-secondary)] bg-[var(--color-surface-sunken)] hover:bg-[var(--color-border)] rounded-full transition-all">Edit</button>
                    <button onClick={(e) => { e.stopPropagation(); handleResume(task); }} className="flex-1 px-4 py-2 text-sm font-semibold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] hover:bg-[var(--color-primary-accent-hover)] rounded-full transition-all shadow-sm">Resume</button>
  ...
  return (
    <main className="container mx-auto p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            className="p-2 border border-[var(--color-border)] rounded-full text-sm font-semibold focus:ring-2 focus:ring-[var(--color-primary-accent)] transition-shadow bg-transparent"
          >
      ...
      {sortedTasks.length > 0 ? (
        ...
        <div className="text-center py-20 bg-[var(--color-surface)] rounded-[var(--border-radius-2xl)] shadow-sm border border-[var(--color-border)]">
          <h2 className="text-2xl font-semibold text-[var(--color-text-primary)]">No Saved Maps Yet</h2>
          <p className="text-[var(--color-text-secondary)] mt-2">Go to your Momentum Map and use the save button to save a map for later.</p>
        </div>
      )}
    </main>
  );
