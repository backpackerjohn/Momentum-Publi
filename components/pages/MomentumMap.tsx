...
import { MomentumMapData, FinishLine, Chunk, SubStep, Note, EnergyTag, Reflection, SavedTask, CompletionRecord, TimeLearningSettings, UserDifficulty, BrainDumpItem } from '../../types';
...
interface MomentumMapProps {
  ...
  initialGoal: { goal: string; context: BrainDumpItem[] } | null;
  setInitialGoal: (goal: null) => void;
}


const MomentumMap: React.FC<MomentumMapProps> = ({ activeMap, setActiveMap, setSavedTasks, completionHistory, onNewCompletionRecord, timeLearningSettings, onSuccess, initialGoal, setInitialGoal }) => {
    ...
    useEffect(() => {
        if (activeMap) {
            ...
            handleGenerateInitialPlan(initialGoal.goal);
            setInitialGoal(null);
        }
    }, [activeMap, initialGoal]);
    ...
    const renderNextBestMoveRibbon = () => {
        ...
        return (
            <div 
                ...
                className={`sticky top-20 z-40 mb-8 p-4 rounded-xl shadow-lg border flex items-center space-x-4 transition-all duration-300 ${isAllDone ? 'bg-green-100 border-green-300' : 'bg-[var(--color-surface)] border-[var(--color-border-hover)] hover:shadow-xl hover:-translate-y-1 cursor-pointer'}`}
            >
    ...
    const renderFinishLine = () => (
         <div className="bg-[var(--color-surface)] p-6 rounded-[var(--border-radius-2xl)] shadow-lg border border-[var(--color-border)] mb-8 relative">
            ...
            {isDirty && (
                <div className="mt-4 pt-4 border-t flex justify-end">
                    <button 
                        ...
                        className="px-5 py-2.5 font-semibold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] rounded-full hover:bg-[var(--color-primary-accent-hover)] transition-all shadow-md disabled:bg-stone-400 flex items-center"
                    >
