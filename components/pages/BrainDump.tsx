...
const categoryColors: { [key: string]: string } = {
    'work': 'bg-green-100 text-green-800', 'personal': 'bg-pink-100 text-pink-800', 'ideas': 'bg-yellow-100 text-yellow-800', 'marketing': 'bg-blue-100 text-blue-800', 'urgent': 'bg-red-100 text-red-800', 'default': 'bg-stone-200 text-stone-800'
};
...
interface BrainDumpProps {
    ...
    onPromote: (goal: string, context: BrainDumpItem[]) => void;
}
...
const BrainDump: React.FC<BrainDumpProps> = ({
    ...
    const renderItem = (item: BrainDumpItem) => {
        ...
        return (
            <div key={item.id} className={`group relative bg-[var(--color-surface)] p-4 rounded-[var(--border-radius-xl)] shadow-sm border transition-all duration-200 flex items-start space-x-4 ${isSelected ? 'shadow-md border-[var(--color-primary-accent)]' : 'border-[var(--color-border)] hover:shadow-md'}`}>
                ...
                <div className="flex-1">
                         ...
                         <div className="mt-2 space-y-2">
                             ...
                             <div className="flex justify-between items-center">
                                 ...
                                 <div className="flex space-x-2">
                                    <button onClick={() => setEditingNoteId(null)} className="px-3 py-1 text-sm font-semibold text-[var(--color-text-secondary)] bg-[var(--color-surface-sunken)] hover:bg-[var(--color-border)] rounded-full transition-colors">Cancel</button>
                                    <button onClick={() => handleSaveNote(item.id)} className="px-3 py-1 text-sm font-semibold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] hover:bg-[var(--color-primary-accent-hover)] rounded-full transition-colors">Save</button>
                                 </div>
                             </div>
                         </div>
                    )}
                    <div className="flex items-center gap-2 mt-3 flex-wrap">
                        ...
                        <input type="text" placeholder="+ Add tag" onKeyDown={(e) => { if (e.key === 'Enter') { const v = e.currentTarget.value.trim(); if (v) { handleAddNewTag(item.id, v); e.currentTarget.value = ''; } } }} className="text-xs px-2 py-1 border border-dashed border-[var(--color-border)] rounded-full w-24 focus:w-32 focus:ring-1 focus:ring-[var(--color-primary-accent)] focus:border-[var(--color-primary-accent)] transition-all bg-transparent"/>
        ...
        <div className="fixed inset-y-0 right-0 w-full md:w-1/3 xl:w-1/4 bg-[var(--color-surface)] shadow-2xl z-50 transform transition-transform duration-300 ease-in-out translate-x-0 border-l border-[var(--color-border)] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {suggestions?.refinements.map(ref => {
                    ...
                    return (
                        <div key={ref.itemId} className="bg-[hsla(var(--color-warning-h),_var(--color-warning-s),_90%,_0.5)] p-3 rounded-[var(--border-radius-lg)] border border-[hsla(var(--color-warning-h),_var(--color-warning-s),_80%,_0.5)]">
            ...
            <div className="p-4 border-t bg-[var(--color-surface)] space-y-2">
                 <button onClick={handleApplyAllSuggestions} className="w-full px-4 py-2 font-semibold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] rounded-full hover:bg-[var(--color-primary-accent-hover)] transition-all">Apply All Suggestions</button>
                 <button onClick={() => setIsSuggestionTrayOpen(false)} className="w-full px-4 py-2 font-semibold text-[var(--color-text-secondary)] bg-[var(--color-surface-sunken)] hover:bg-[var(--color-border)] rounded-full transition-all">Dismiss</button>
            </div>
        </div>
    );
    ...
    return (
        <main className="container mx-auto p-8">
            <div className={`transition-all duration-300 ${isSuggestionTrayOpen ? 'max-w-4xl' : 'max-w-4xl mx-auto'}`}>
                ...
                <p className="text-[var(--color-text-secondary)] mb-6">A zero-judgment capture zone. Unload your thoughts, and we'll help you find the momentum.</p>

                <div className="bg-[hsl(35,45%,90%)] p-6 rounded-[var(--border-radius-2xl)] shadow-lg border border-[var(--color-border)]">
                    <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Dump your thoughts here... nothing is judged." className="w-full h-48 p-4 bg-[var(--color-surface-sunken)] border border-[var(--color-border-hover)] rounded-[var(--border-radius-lg)] focus:ring-2 focus:ring-[var(--color-primary-accent)] transition-shadow resize-none" />
                    <div className="mt-4 flex justify-end items-center gap-2">
                        <button onClick={handleClear} className="px-6 py-3 font-semibold text-[var(--color-text-secondary)] bg-transparent hover:bg-black/5 rounded-full transition-all">
                            Clear
                        </button>
                        <button onClick={handleProcess} disabled={isLoading} className="px-6 py-3 font-semibold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] rounded-full hover:bg-[var(--color-primary-accent-hover)] transition-all shadow-md disabled:bg-stone-400 flex items-center">
                ...
                {processedItems.length > 0 && (
                    <div className="mt-8">
                        <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
                            <h2 className="text-3xl font-bold text-[var(--color-text-primary)]">Captured Thoughts</h2>
            ...
            {selectedItems.length > 0 && (
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 bg-[var(--color-surface)] shadow-2xl rounded-2xl p-4 flex items-center gap-4 border border-[var(--color-border)] animate-fade-in">
                    <p className="font-semibold text-[var(--color-text-primary)]">{selectedItems.length} item{selectedItems.length > 1 ? 's' : ''} selected</p>
                    <button onClick={() => setIsPromoteModalOpen(true)} className="px-4 py-2 text-sm font-bold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] hover:bg-[var(--color-primary-accent-hover)] rounded-lg shadow-md">
                        ✨ Promote to Map
                    </button>
                    <button onClick={() => setSelectedItems([])} className="p-1 rounded-full text-stone-500 hover:bg-stone-200">
                        <XIcon className="h-5 w-5" />
                    </button>
                </div>
            )}
            {isPromoteModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setIsPromoteModalOpen(false)}>
                    <div className="bg-[var(--color-surface)] rounded-2xl shadow-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
                        <h2 className="text-2xl font-bold text-[var(--color-text-primary)] mb-2">Promote to Momentum Map</h2>
                        <p className="text-[var(--color-text-secondary)] mb-4">Which of these is your main goal? The others will be added as context for the AI to build your plan.</p>
                        <PromoteModal
                            items={processedItems.filter(i => selectedItems.includes(i.id))}
                            onPromote={(goal, context) => {
                                onPromote(goal, context);
...
const PromoteModal: React.FC<{ items: BrainDumpItem[], onPromote: (goal: string, context: BrainDumpItem[]) => void }> = ({ items, onPromote }) => {
    const [mainGoalId, setMainGoalId] = useState<string>(items[0]?.id || '');

    const handleSubmit = () => {
        const mainGoal = items.find(i => i.id === mainGoalId);
        const contextItems = items.filter(i => i.id !== mainGoalId);
        if (mainGoal) {
            onPromote(mainGoal.item, contextItems);
        }
    };
    ...
    return (
        <div>
            <div className="space-y-2 max-h-60 overflow-y-auto border rounded-lg p-2">
                {items.map(item => (
                    <label key={item.id} className={`flex items-center p-3 rounded-lg cursor-pointer transition-colors ${mainGoalId === item.id ? 'bg-[var(--color-surface-sunken)]' : 'hover:bg-[var(--color-surface-sunken)]'}`}>
                        <input
                            type="radio"
                            name="main-goal"
                            value={item.id}
                            checked={mainGoalId === item.id}
                            onChange={(e) => setMainGoalId(e.target.value)}
                            className="h-4 w-4 text-[var(--color-primary-accent)] focus:ring-[var(--color-primary-accent)]"
                        />
                        <span className="ml-3 text-sm font-medium text-[var(--color-text-primary)]">{item.item}</span>
                    </label>
                ))}
            </div>
            <div className="mt-6 flex justify-end">
                <button onClick={handleSubmit} className="px-6 py-2 font-bold text-[var(--color-primary-accent-text)] bg-[var(--color-primary-accent)] rounded-lg hover:bg-[var(--color-primary-accent-hover)]">
                    Create Map
                </button>
            </div>
        </div>
