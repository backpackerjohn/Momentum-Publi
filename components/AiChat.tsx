            ...
            properties: {
                ...
                parameters: {
                    ...
                    properties: {
                        ...
                        days: { type: Type.ARRAY, items: { type: Type.STRING, enum: DAYS_OF_WEEK } },
...
const AiChat: React.FC<AiChatProps> = (props) => {
    ...
    const processAction = (action: any) => {
        ...
        if (name === 'ADD_ANCHOR' && parameters.days) {
            ...
            const dayIndices = parameters.days.map((d: string) => DAYS_OF_WEEK.indexOf(d));

            const newEvent: ScheduleEvent = {
                id: `ai-${Date.now()}`, 
                days: dayIndices, 
                title: parameters.title,
                startMin: timeToMinutes(parameters.startTime), 
                endMin: timeToMinutes(parameters.endTime),
                contextTags: [ContextTag.Personal]
            };
            props.setScheduleEvents(prev => [...prev, newEvent]);
            
            const dayStr = formatDaysForToast(dayIndices);
            ...
            card = { type: 'anchor', title: parameters.title, details: [`${formatDaysForToast(dayIndices)}`, `${formatTimeForToast(parameters.startTime)} - ${formatTimeForToast(parameters.endTime)}`] };
            ...
            if (targetAnchors.length > 0) {
                ...
                const newReminders: SmartReminder[] = targetAnchors.map(anchor => ({
                    id: `ai-sr-${anchor.id}-${Date.now()}`, 
                    anchorId: anchor.id,
                    offsetMinutes: parameters.offsetMinutes, 
                    message: parameters.reminderMessage,
                    why: `Set by you via AI assistant.`, 
                    isLocked: false, 
                    isExploratory: false,
